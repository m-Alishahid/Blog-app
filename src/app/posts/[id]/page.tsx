...
export default function Post({ params }: { params: Promise<{ id: string }> }) {
  ...
  React.useEffect(() => {
    const fetchParams = async () => {
      ...
      setPostId(resolvedParams.id); // Set the id to the state
    };
  ...
  if (!postId) {
    return <h2 className="text-2xl font-semibold text-center  mt-72 mb-72">Loading...</h2>;
  }
  ...
  if (!post) {
    return <h2 className="text-2xl font-semibold text-center mt-10">Post Not Found</h2>;
  }
